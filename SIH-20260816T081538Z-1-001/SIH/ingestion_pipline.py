...
Usage
-----
    python era5_wind_wave_to_tensor.py \\
        --oper data_stream-oper_stepType-instant.nc \\
        --wave data_stream-wave_stepType-instant.nc \\
        --cmems cmems_currents_ssh.nc \\
        --bbox 10.0,20.0,80.0,95.0 \\
        --output ocean_weather_tensor.pt
...
def run(
    oper_path: str, wave_path: str, bbox: BoundingBox, output_path: str, cmems_path: Optional[str] = None
) -> torch.FloatTensor:
    """
    Execute the full oper+wave(+cmems) -> tensor pipeline end to end.
    ...
    logger.info("=== Step 2: Slicing to first timestep ===")
    oper_t0 = slice_first_timestep(oper_raw)
    wave_t0 = slice_first_timestep(wave_raw)
    cmems_t0 = slice_first_timestep(cmems_raw) if cmems_raw is not None else None

    logger.info("=== Step 3: Standardizing coordinates + merging ===")
    oper_std = standardize_coords(oper_t0)
    wave_std = standardize_coords(wave_t0)
    cmems_std = standardize_coords(cmems_t0) if cmems_t0 is not None else None
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Path to the ERA5 oper-stream .nc file (u10, v10). Default: data_stream-oper_stepType-instant.nc",
    )
    parser.add_argument(
        "--wave", type=str, default="C:/Users/Admin/OneDrive/Desktop/SIH/SIH data/data_stream-wave_stepType-instant.nc",
        help="Path to the ERA5 wave-stream .nc file (swh, mwp, mwd). Default: data_stream-wave_stepType-instant.nc",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Output .pt filepath (default: ocean_weather_tensor.pt).",
    )
    return parser
...
if __name__ == "__main__":
    ...
    run(oper_path=args.oper, wave_path=args.wave, bbox=args.bbox, output_path=args.output, cmems_path=args.cmems)